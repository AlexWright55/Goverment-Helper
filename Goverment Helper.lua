...
local script_tag = '[Goverment Helper]'
...
local tagReplacements = {
    ...
    get_storecar_model = function()
        ...
        if closest_car then
            ...
            sampAddChatMessage(
                "[Goverment Helper] {ffffff}Не удалось получить модель ближайшего транспорта с водителем!",
                0x6E6E6E)
            return ''
        end
    end,
...
function welcome_message()
    ...
    sampAddChatMessage(script_tag ..
                       ...
                       message_color)
    show_cef_notify('info', 'Goverment Helper',
                ...
                lua_thread.create(function()
                    ...
                    for line_index, line in ipairs(lines) do
                        ...
                        elseif line == "{pause}" then
                            ...
                            if debug_mode then
                                sampAddChatMessage(
                                    '[Goverment Helper DEBUG] {ffffff}SEND: ' ..
                                        line, message_color)
                            end
                        end
                    end
...
function play_error_sound()
    ...
    show_cef_notify('error', 'Goverment Helper',
...
function check_update()
        ...
        downloadToFile(url, path, function(type, pos, total_size)
            if type == "finished" then
                ...
                if updateInfo then
                    ...
                    print(
                        "[Goverment Helper] Текущая установленная версия:",
                        thisScript().version)
                    print(
                        "[Goverment Helper] Текущая версия в облаке:",
                        uVer)
    ...
    function readJsonFile(filePath)
        if not doesFileExist(filePath) then
            print("[Goverment Helper] Ошибка: Файл " .. filePath ..
                      " не существует")
            return nil
        end
        ...
        if not file then
            print(
                "[Goverment Helper] Ошибка при открытии файла " ..
                    filePath .. ": " .. err)
            return nil
        end
        ...
        if not jsonData then
            print(
                "[Goverment Helper] Ошибка: Неверный формат JSON в файле " ..
                    filePath)
            return nil
        end
...
function sampev.onShowDialog(dialogid, style, title, button1, button2, text)
    ...
    if members_check and title:find('(.+)%(В сети: (%d+)%)') then -- мемберс 
        ...
        elseif #members_new ~= 0 then
            ...
            sampSendDialogResponse(dialogid, 0, 0, 0)
            sampAddChatMessage(
                '[Goverment Helper]{ffffff} Список сотрудников пуст!',
                message_color)
            members_check = false
        end
        return false
...
imgui.OnFrame(function() return MainWindow[0] end, function(player)
    ...
    if imgui.BeginTabBar('пон') then
        ...
        if imgui.BeginTabItem(fa.STAR .. u8 'Положение') then
            if imgui.BeginChild('##smartRPTP', imgui.ImVec2(
                                    690 * MONET_DPI_SCALE, 354 * MONET_DPI_SCALE),
                ...
                if imgui.Button(fa.DOWNLOAD ..
                    ...
                    if getARZServerNumber() ~= 0 then
                        ...
                        downloadFileFromUrlToPath(
                            'https://raw.githubusercontent.com/Alexandr-Botovod/Goverment_Helper/refs/heads/main/GovermentHelper/' ..
                        ...
                        imgui.OpenPopup(fa.CIRCLE_INFO ..
                                            u8 ' Goverment Helper - Оповещение##donwloadsmartRPTP')
                    else
                        imgui.OpenPopup(fa.CIRCLE_INFO ..
                                            u8 ' Goverment Helper - Оповещение##nocloudsmartRPTP')
                    end
                end
                if imgui.BeginPopupModal(fa.CIRCLE_INFO ..
                    ...
                    if imgui.Button(fa.CIRCLE_XMARK .. u8 ' Хорошо',
                                    imgui.ImVec2(550 * MONET_DPI_SCALE,
                                                 25 * MONET_DPI_SCALE)) then
                        imgui.CloseCurrentPopup()
                    end
                    imgui.EndPopup()
                ...
                if imgui.BeginPopupModal(fa.STAR ..
                                         ...
                                         _, imgui.WindowFlags.NoCollapse +
                                             imgui.WindowFlags.NoResize) then
                    imgui.BeginChild('##smartRPTPedit', imgui.ImVec2(
                                         589 * MONET_DPI_SCALE,
                                         360 * MONET_DPI_SCALE), true)
                    for chapter_index, chapter in ipairs(smart_rptp) do
                        ...
                        if imgui.Button(fa.TRASH_CAN .. '##' .. chapter_index) then
                            imgui.OpenPopup(
                                fa.TRIANGLE_EXCLAMATION ..
                                    u8 ' Goverment Helper - Предупреждение ##' ..
                                    chapter_index)
                        end
                        if imgui.BeginPopupModal(
                            fa.TRIANGLE_EXCLAMATION ..
        ...
        if imgui.BeginTabItem(fa.GEAR .. u8 ' Настройки') then
            ...
            if imgui.BeginPopupModal(fa.TRIANGLE_EXCLAMATION ..
                                     ...
                                     _, imgui.WindowFlags.NoResize) then
                imgui.CenterText(
                    u8 'Вы действительно хотите удалить Goverment Helper?')
                imgui.Separator()
                if imgui.Button(fa.CIRCLE_XMARK ..
                                    u8 ' Нет, отменить',
                                imgui.ImVec2(200 * MONET_DPI_SCALE,
...
imgui.OnFrame(function() return GiveRankMenu[0] end, function(player)
    imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2),
                           imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
    imgui.Begin(fa.BUILDING_SHIELD .. " Goverment Helper##rank", GiveRankMenu,
                imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize +
                    imgui.WindowFlags.NoScrollbar +
                    imgui.WindowFlags.AlwaysAutoResize)
                ...
                lua_thread.create(function()
                    ...
                    for _, line in ipairs(lines) do
                        ...
                        if wait_tag then
                            ...
                            if debug_mode then
                                sampAddChatMessage(
                                    '[Goverment Helper DEBUG] Отправляю сообщение: ' ..
                                        line, message_color)
                            end
...
imgui.OnFrame(function() return CommandStopWindow[0] end, function(player)
    imgui.SetNextWindowPos(
        ...
        imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
    imgui.Begin(fa.BUILDING_SHIELD .. " Goverment Helper##CommandStopWindow", _,
                imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize +
                    imgui.WindowFlags.NoScrollbar +
                    imgui.WindowFlags.AlwaysAutoResize)
...
imgui.OnFrame(function() return CommandPauseWindow[0] end, function(player)
    imgui.SetNextWindowPos(
        imgui.ImVec2(sizeX / 2, sizeY - 50 * MONET_DPI_SCALE),
        imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
    imgui.Begin(fa.BUILDING_SHIELD .. " Goverment Helper##CommandPauseWindow",
...
imgui.OnFrame(function() return FastMenuButton[0] end, function(player)
    imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 8.5, sizeY / 2.3),
                           imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
    imgui.Begin(fa.HOSPITAL .. " Goverment Helper##fast_menu_button",
                FastMenuButton,
                imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize +
